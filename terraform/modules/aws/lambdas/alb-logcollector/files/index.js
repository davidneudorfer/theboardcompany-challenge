...
var parse = require('alb-log-parser');  // Apache Common Log Format
...
/* Globals */
var options = {
    endpoint: 'https://challenge-logcollector.herokuapp.com/log/ingest',
};

var endpoint =  new AWS.Endpoint(options.endpoint);
var s3 = new AWS.S3();
...
function postDocumentToES(doc, context) {
    ...
    req.method = 'POST';
    // req.path = path.join('/', options.index, options.doctype);
...
exports.handler = function(event, context) {
    ...
    */
    ...
    recordStream._transform = function(line, encoding, done) {
        ...
        var decimal_timestamp = new Date(logRecord.timestamp).getTime();
        var customlines = {
            client_ip: logRecord.client,
            domain_name: logRecord.request_uri_host,
            raw_line: line.toString(),
            request_method: logRecord.request_method,
            request_protocol: logRecord.request_uri_scheme,
            request_protocol_version: logRecord.request_http_version,
            request_url: logRecord.request_uri,
            status_code: logRecord.target_status_code,
            timestamp: decimal_timestamp
        };
        var optimized = {
            collection: "theboardcompany",
            lines: [
                customlines
            ]
        };
        var serializedRecord = JSON.stringify(optimized);
        // print each line
